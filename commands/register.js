				...
				if(key == faction) {
					...
					request({
						...
						if(!err && res.statusCode == 200) {
							...
							// Check if member is in an outfit_id if not break
							if(clist.outfit_id == 'undefined') return(message.channel.send('<'+message.author+'> Sorry I couldn\'t find a user in an outfit with that username. Need help? Try !hep register.'));
